...
#include "sensing-max9295.h"

/* register specifics */

#define MAX9295_TX_STR_SEL_PIPE_X 0x53
#define MAX9295_TX_STR_SEL_PIPE_Y 0x57
#define MAX9295_TX_STR_SEL_PIPE_Z 0x5B
#define MAX9295_TX_STR_SEL_PIPE_U 0x5F
...
static int max9295_read_reg(struct device *dev, u16 addr, u8 *val) {
  ...
  *val = reg_val & 0xFF;
  // printk("max9295-read-addr = %x, val = %x\n", addr, *val);
  return err;
}
...
static int max9295_write_reg(struct device *dev, u16 addr, u8 val) {
  ...
  int err;
  u8 e;
  // printk("max9295-write-addr = %x, val = %x\n", addr, val);
...
int max9295_setup_streaming(struct device *dev) {
  ...
  struct map_ctx map_pipe_dtype[] = {
      {GMSL_CSI_DT_RAW_12, MAX9295_PIPE_Z_DT_ADDR, 0x12, MAX9295_ST_ID_2},
      {GMSL_CSI_DT_UED_U1, MAX9295_PIPE_X_DT_ADDR, 0x30, MAX9295_ST_ID_0},
      {GMSL_CSI_DT_YUV_8, MAX9295_PIPE_Y_DT_ADDR, 0x1E, MAX9295_ST_ID_1},
  };
  ...
  for (i = 0; i < g_ctx->num_streams; i++) {
    ...
    for (j = 0; j < ARRAY_SIZE(map_pipe_dtype); j++) {
      if (map_pipe_dtype[j].dt == g_stream->st_data_type) {
        ...
        st_en =
            (map_pipe_dtype[j].addr == MAX9295_PIPE_X_DT_ADDR) ? 0xC0 : 0x40;
  ...
  pipe_en = (MAX9295_VID_TX_EN_X | MAX9295_VID_TX_EN_Y | MAX9295_VID_TX_EN_Z |
             MAX9295_VID_TX_EN_U | MAX9295_VID_INIT);
  ...
  // NOTICE: suggested according camera register guide
  // 0x2D3 control mfp7 as one shot trigger.
  max9295_write_reg(dev, 0x2D3, 0x00);
  mdelay(500);
  max9295_write_reg(dev, 0x2D3, 0x10);
...
int max9295_setup_control(struct device *dev) {
  ...
  // NOTICE: add according to sensing's register guide but don't know the meaning.
  // max9295_write_reg(dev, MAX9295_TX_STR_SEL_PIPE_Y, 0x12);
  // max9295_write_reg(dev, MAX9295_TX_STR_SEL_PIPE_Z, 0x11);
...
static int max9295_probe(struct i2c_client *client,
  ...
  if (of_get_property(node, "is-prim-ser", NULL)) {
    // if (prim_priv__) {
    if (prim_priv__[channel_count] && channel_count >= MAX_CHANNEL_NUM) {
    ...
    // prim_priv__ = priv;
    prim_priv__[channel_count] = priv;
    channel_count++;
  }
...
static void __exit max9295_exit(void) { i2c_del_driver(&max9295_i2c_driver); }
