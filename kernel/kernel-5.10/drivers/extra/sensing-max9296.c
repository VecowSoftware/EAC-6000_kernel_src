...
#define MAX9296_PHY1_CLK 0x2F // 0x2F equals to 1500MHz (1.5Gbps)
...
static int max9296_get_sdev_idx(struct device *dev, struct device *s_dev,
                                int *idx) {
  struct max9296 *priv = dev_get_drvdata(dev);
  int i;
  int err = 0;
  ...
  for (i = 0; i < priv->max_src; i++) {
    if (priv->sources[i].g_ctx->s_dev == s_dev) break;
  }
  if (i == priv->max_src) {
...
static void max9296_pipes_reset(struct max9296 *priv) {
  ...
  struct pipe_ctx pipe_defaults[] = {
      {MAX9296_PIPE_X, GMSL_CSI_DT_EMBED, MAX9296_CSI_CTRL_1, 0,
       MAX9296_INVAL_ST_ID},
      {MAX9296_PIPE_Y, GMSL_CSI_DT_YUV_8, MAX9296_CSI_CTRL_1, 0,
       MAX9296_INVAL_ST_ID},
      {MAX9296_PIPE_Z, GMSL_CSI_DT_YUV_8, MAX9296_CSI_CTRL_1, 0,
       MAX9296_INVAL_ST_ID}};
...
static void max9296_reset_ctx(struct max9296 *priv) {
  ...
  for (i = 0; i < priv->num_src; i++) priv->sources[i].st_enabled = false;
}
...
int max9296_power_on(struct device *dev) {
  ...
  if (priv->pw_ref == 0) {
    usleep_range(1, 2);
    if (priv->reset_gpio) gpio_direction_output(priv->reset_gpio, 0);

    usleep_range(50, 80);
    if (priv->vdd_cam_1v2) {
      err = regulator_enable(priv->vdd_cam_1v2);
      if (unlikely(err)) goto ret;
    }
    ...
    dev_info(dev, "waiting for a while for camera power up\n");
    msleep(2000);
  }
...
void max9296_power_off(struct device *dev) {
  ...
  if (priv->pw_ref == 0) {
    ...
    if (priv->vdd_cam_1v2) regulator_disable(priv->vdd_cam_1v2);
  }
...
int max9296_setup_link(struct device *dev, struct device *s_dev) {
  ...
  if (err) {
    dev_err(dev, "[%s] : max9296_get_sdev_idx failed.\n", __func__);
    return err;
  }
  ...
  if (!priv->splitter_enabled) {
    ...
    if (err) {
      dev_err(dev, "[%s] : max9296_write_link failed.\n", __func__);
      goto ret;
    }
...
static int max9296_link_locked(struct device *dev) {
  u8 val;
  usleep_range(100, 110);
  max9296_read_reg(dev, MAX9296_LINK_ADDR, &val);
  if (0 == (val & 0x08)) {
    return -EINVAL;
  }
  return 0;
...
int max9296_setup_control(struct device *dev, struct device *s_dev) {
  ...
  // enable deskew calibration
  max9296_write_reg(dev, MAX9296_MIPI_TX3, 0x80);
...
/*
static int max9296_get_available_pipe(struct device *dev, u32 st_data_type,
                                      u32 dst_csi_port) {
  struct max9296 *priv = dev_get_drvdata(dev);
  int i;
  ...
  for (i = 0; i < MAX9296_MAX_PIPES; i++) {
    if ((priv->pipe[i].dt_type == st_data_type) &&
        ((dst_csi_port == GMSL_CSI_PORT_A)
             ? (priv->pipe[i].dst_csi_ctrl == MAX9296_CSI_CTRL_0) ||
                   (priv->pipe[i].dst_csi_ctrl == MAX9296_CSI_CTRL_1)
...
static int max9296_setup_pipeline(struct device *dev,
  ...
  for (i = 0; i < g_ctx->num_streams; i++) {
    ...
    if (g_stream->st_data_type == GMSL_CSI_DT_YUV_8) {
      map_list = map_pipe_yuv8;
      arr_sz = ARRAY_SIZE(map_pipe_yuv8);
    } else if (g_stream->st_data_type == GMSL_CSI_DT_RAW_12) {
      ...
      dev_dbg(dev, "%s: No mapping for GMSL_CSI_DT_UED_U1\n", __func__);
      continue;
    } else {
    ...
    // pipe_id = max9296_get_available_pipe(dev, g_stream->st_data_type,
                                        //  g_ctx->dst_csi_port);
    if (g_ctx->serdes_csi_link == GMSL_SERDES_CSI_LINK_A)
      pipe_id = 1;
    else
      pipe_id = 2;
    dev_info(dev, "selected pipeline id: %d", pipe_id);
    ...
    for (j = 0, vc_idx = 3; j < arr_sz; j++, vc_idx += 2) {
      ...
      max9296_write_reg(dev, map_list[j].addr, map_list[j].val);
    }
...
int max9296_start_streaming(struct device *dev, struct device *s_dev) {
  ...
  for (i = 0; i < g_ctx->num_streams; i++) {
    ...
    if (g_stream->des_pipe != MAX9296_PIPE_INVALID) {
      max9296_write_reg(dev, g_stream->des_pipe, g_stream->st_id_sel);
    }
  }
  mutex_unlock(&priv->lock);
...
int max9296_stop_streaming(struct device *dev, struct device *s_dev) {
  ...
  for (i = 0; i < g_ctx->num_streams; i++) {
    ...
    if (g_stream->des_pipe != MAX9296_PIPE_INVALID) {
      max9296_write_reg(dev, g_stream->des_pipe, MAX9296_RESET_ST_ID);
    }
  }
...
int max9296_setup_streaming(struct device *dev, struct device *s_dev) {
  ...
  mutex_lock(&priv->lock);
  if (priv->sources[i].st_enabled) goto ret;
  ...
  if (!priv->lane_setup) {
    max9296_write_reg(dev, MAX9296_DST_CSI_MODE_ADDR, priv->csi_mode);
    max9296_write_reg(dev, MAX9296_LANE_MAP1_ADDR, priv->lane_mp1);
    max9296_write_reg(dev, MAX9296_LANE_MAP2_ADDR, priv->lane_mp2);
    max9296_write_reg(dev, MAX9296_PHY1_CLK_ADDR, MAX9296_PHY1_CLK);

    priv->lane_setup = true;
  }

  max9296_write_reg(dev, MAX9296_REG5_ADDR, 0x00);
...
const struct of_device_id max9296_of_match[] = {
    {
        .compatible = "sensing,max9296",
    },
    {},
};
MODULE_DEVICE_TABLE(of, max9296_of_match);

static int max9296_parse_dt(struct max9296 *priv, struct i2c_client *client) {
  ...
  if (of_get_property(node, "vdd_cam_1v2-supply", NULL)) {
    ...
    if (IS_ERR(priv->vdd_cam_1v2)) {
      dev_err(&client->dev, "vdd_cam_1v2 regulator get failed\n");
...
static int max9296_probe(struct i2c_client *client,
  ...
  priv->regmap = devm_regmap_init_i2c(priv->i2c_client, &max9296_regmap_config);
  ...
  if (priv->max_src > MAX9296_MAX_SOURCES) {
    dev_err(&client->dev, "max sources more than currently supported\n");
    return -EINVAL;
  }
...
static void __exit max9296_exit(void) { i2c_del_driver(&max9296_i2c_driver); }
