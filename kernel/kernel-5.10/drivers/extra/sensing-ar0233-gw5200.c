/*
 * sensing-ar0233-gw5200.c - ar0233 sensor driver
 *
...
#include "sensing-ar0233_mode_tbls.h"
...
static const u32 ctrl_cid_list[] = {
    ...
    TEGRA_CAMERA_CID_HDR_EN,         TEGRA_CAMERA_CID_SENSOR_MODE_ID,
};
...
static inline int ar0233_read_reg(struct camera_common_data *s_data, u16 addr,
                                  u16 *val) {
  int err = 0;
  u32 reg_val = 0;
...
static int ar0233_write_reg(struct camera_common_data *s_data, u16 addr,
                            u16 val) {
  int err;
...
static int ar0233_write_table(struct ar0233 *priv, const ar0233_reg table[]) {
  int ret = 0;
  int i = 0;
  int retry;
...
static inline void ar0233_get_gain_reg(ar0233_reg *regs, u16 gain) {
...
static int ar0233_set_gain(struct tegracam_device *tc_dev, s64 val) {
  ...
  dev_dbg(dev, "%s: db: %d\n", __func__, gain_reg);
  if (gain > AR0233_MAX_GAIN_REG) gain = AR0233_MAX_GAIN_REG;
  ar0233_get_gain_reg(reg_list, gain_reg);
  err = ar0233_write_reg(s_data, reg_list[0].addr, reg_list[0].val);
  if (err) goto fail;
  return 0;
fail:
...
static int ar0233_set_exposure(struct tegracam_device *tc_dev, s64 val) {
  ...
  if (priv->frame_length == 0) priv->frame_length = AR0233_DEFAULT_FRAME_LENGTH;
  ...
  for (i = 0; i < 1; i++) {
    err = ar0233_write_reg(priv->s_data, reg_list[i].addr, reg_list[i].val);
    if (err) goto fail;
  }
...
static struct camera_common_pdata *ar0233_parse_dt(
    struct tegracam_device *tc_dev) {
  struct device *dev = tc_dev->dev;
  ...
  if (err) dev_err(dev, "mclk not in DT\n");

  return board_priv_pdata;
}

static int ar0233_set_mode(struct tegracam_device *tc_dev) {
  // We don't need set_mode. Keep this function only for compatibility.
  return 0;
}
...
static const struct i2c_device_id ar0233_id[] = {{"sensing,ar0233isp", 0}, {}};
